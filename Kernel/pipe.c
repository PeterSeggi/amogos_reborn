...
int read_pipe(int fd, char *buffer, uint16_t amount){
    ...
    while(amount_read < amount && aux_pipe->to_read_idx!=aux_pipe->to_write_idx){
        ...
        aux_pipe->to_read_idx = (aux_pipe->to_read_idx+1)%BUFFER_SIZE;
...
int write_pipe(int fd, char *message, uint16_t length){
    ...
    while(amount_written < length){
        ...
        aux_pipe->to_write_idx = (aux_pipe->to_write_idx+1)%BUFFER_SIZE;
        sem_post(aux_pipe->sem_mutex);
...
*/
pipe_t *create_pipe(int read_fd, int write_fd){
    pipe_t *aux_pipe = (pipe_t *) my_malloc(sizeof);
    ...
    set_pipe_name(fd/2, aux_pipe_name);
    strcpy(aux_pipe_name + 8, "_to_read");
    aux_pipe->sem_to_read = sem_open(aux_pipe_name, 0);
    if(!aux_pipe->sem_to_read) return NULL;
    strcpy(aux_pipe_name + 8, "_to_write");
    aux_pipe->sem_to_write = sem_open(aux_pipe_name, BUFFER_SIZE);
    if(!aux_pipe->sem_to_write) return NULL;
    strcpy(aux_pipe_name + 8, "_mutex");
...
pipe_t * check_valid_fd(uint16_t fd, pipe_action action){
    ...
    switch(action){
        case READ:
            return fd==aux_pipe->read_fd;

        case WRITE:
            return fd==aux_pipe->write_fd;
